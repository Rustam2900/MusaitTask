...
Revision ID: bfe8389d6d30
Revises: 
Create Date: 2024-12-19 13:23:16.841935
...
def upgrade() -> None:
    ...
    op.drop_index('bot_reminder_user_id_76c71f0b', table_name='bot_reminder')
    op.drop_table('bot_reminder')
    ...
    op.alter_column('bot_user', 'id',
               existing_type=sa.BIGINT(),
               server_default=None,
               type_=sa.Integer(),
               existing_nullable=False,
               autoincrement=True)
    op.alter_column('bot_user', 'full_name',
               existing_type=sa.VARCHAR(length=255),
               nullable=True)
    op.alter_column('bot_user', 'phone_number',
               existing_type=sa.VARCHAR(length=20),
               nullable=True)
    op.drop_index('bot_user_email_eb7b33ec_like', table_name='bot_user')
    op.drop_index('bot_user_phone_number_ed0d6ddc_like', table_name='bot_user')
    op.drop_index('bot_user_telegram_id_ee049d50_like', table_name='bot_user')
    op.drop_index('bot_user_tg_username_dec95e2c_like', table_name='bot_user')
    op.drop_column('bot_user', 'date_joined')
    op.drop_column('bot_user', 'full_name_ru')
    op.drop_column('bot_user', 'last_name')
    op.drop_column('bot_user', 'is_staff')
    op.drop_column('bot_user', 'is_active')
    op.drop_column('bot_user', 'username_uz')
    op.drop_column('bot_user', 'first_name')
    op.drop_column('bot_user', 'username_ru')
    op.drop_column('bot_user', 'last_login')
    op.drop_column('bot_user', 'is_superuser')
    op.drop_column('bot_user', 'full_name_uz')
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('bot_user', sa.Column('full_name_uz', sa.VARCHAR(length=255), autoincrement=False, nullable=True))
    op.add_column('bot_user', sa.Column('is_superuser', sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.add_column('bot_user', sa.Column('last_login', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=True))
    op.add_column('bot_user', sa.Column('username_ru', sa.VARCHAR(length=255), autoincrement=False, nullable=True))
    op.add_column('bot_user', sa.Column('first_name', sa.VARCHAR(length=150), autoincrement=False, nullable=False))
    op.add_column('bot_user', sa.Column('username_uz', sa.VARCHAR(length=255), autoincrement=False, nullable=True))
    op.add_column('bot_user', sa.Column('is_active', sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.add_column('bot_user', sa.Column('is_staff', sa.BOOLEAN(), autoincrement=False, nullable=False))
    op.add_column('bot_user', sa.Column('last_name', sa.VARCHAR(length=150), autoincrement=False, nullable=False))
    op.add_column('bot_user', sa.Column('full_name_ru', sa.VARCHAR(length=255), autoincrement=False, nullable=True))
    op.add_column('bot_user', sa.Column('date_joined', postgresql.TIMESTAMP(timezone=True), autoincrement=False, nullable=False))
    op.create_index('bot_user_tg_username_dec95e2c_like', 'bot_user', ['tg_username'], unique=False)
    op.create_index('bot_user_telegram_id_ee049d50_like', 'bot_user', ['telegram_id'], unique=False)
    op.create_index('bot_user_phone_number_ed0d6ddc_like', 'bot_user', ['phone_number'], unique=False)
    op.create_index('bot_user_email_eb7b33ec_like', 'bot_user', ['email'], unique=False)
    op.alter_column('bot_user', 'phone_number',
               existing_type=sa.VARCHAR(length=20),
               nullable=False)
    op.alter_column('bot_user', 'full_name',
    ...
    op.create_table('bot_reminder',
    sa.Column('id', sa.BIGINT(), sa.Identity(always=False, start=1, increment=1, minvalue=1, maxvalue=9223372036854775807, cycle=False, cache=1), autoincrement=True, nullable=False),
    sa.Column('title', sa.VARCHAR(length=255), autoincrement=False, nullable=False),
    sa.Column('title_uz', sa.VARCHAR(length=255), autoincrement=False, nullable=True),
    sa.Column('title_ru', sa.VARCHAR(length=255), autoincrement=False, nullable=True),
    sa.Column('content', sa.TEXT(), autoincrement=False, nullable=False),
    sa.Column('content_uz', sa.TEXT(), autoincrement=False, nullable=True),
    ...
    sa.Column('user_id', sa.INTEGER(), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['auth_user.id'], name='bot_reminder_user_id_76c71f0b_fk_auth_user_id', initially='DEFERRED', deferrable=True),
    sa.PrimaryKeyConstraint('id', name='bot_reminder_pkey')
    )
    op.create_index('bot_reminder_user_id_76c71f0b', 'bot_reminder', ['user_id'], unique=False)
